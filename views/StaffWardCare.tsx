...
{beds.map(bed => (
 ...
 <span className={`text-[10px] font-black uppercase tracking-widest ${s.header}`}>
{bed.type === 'GENERAL' ? 'General Ward - Quiet Zone' : 
 bed.type === 'SEMI_PRIVATE' ? 'Semi-Private - Balanced Care' :
 bed.type === 'PRIVATE' ? 'Private Suite - Premium Recovery' :
 bed.type === 'ICU' ? 'ICU - Critical Monitoring' : bed.type}
 </span>
 <span className={`text-[8px] font-bold uppercase tracking-widest opacity-60 ${s.sub}`}>
{bed.type === 'GENERAL' ? '₹2,500 / night' : 
 bed.type === 'SEMI_PRIVATE' ? '₹5,000 / night' :
 bed.type === 'PRIVATE' ? '₹10,000 / night' :
 bed.type === 'ICU' ? '₹15,000 / night' : ''}
 </span>
 </div>
